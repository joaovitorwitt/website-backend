...
class BaseEntity:  # pylint: disable=too-many-instance-attributes
    def __init__(
        self, title: str, description: str, content: str, image_url: str
    ) -> None:
        self.title = format_title_for_displaying(title)
        self.description: str = description
        self.creation_time = datetime.strftime(
            datetime.now(), format='%Y%m%d T%H:%M:%S'
        )
    ...
    def __str__(self) -> str:
        ...
        """
        return f'Title: {self.title!r} || Description: {self.description!r}'
